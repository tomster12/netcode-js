...
function reconcileGameState(localState, serverState) {
    const worldTimeNow = Date.now() - localState.worldTimeStart;
    return serverState;
}

function updateGameState(state, events) {
    const worldTimeNow = Date.now() - state.worldTimeStart;
    const dt = (worldTimeNow - state.lastUpdateWorldTime) / 1000;
    state.lastUpdateWorldTime = worldTimeNow;
